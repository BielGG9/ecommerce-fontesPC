...
public class MarcaServiceImpl implements MarcaService {
    ...
    public MarcaResponse delete(Long id) {
        ...
        // Impedir exclusão se houver modelos atrelados a fim de evitar violação de Foreign Key (Erro 500)
        long vinculados = modeloRepository.count("marca.id", id);
        if (vinculados > 0) {
            throw new WebApplicationException("Não é possível excluir esta Marca, pois há " + vinculados + " modelo(s) vinculado(s) a ela.", Response.Status.BAD_REQUEST);
        }
    ...
    @Override
    public List<MarcaResponse> findAll(int page, int pageSize, String nome) {

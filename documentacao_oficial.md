...
```mermaid
usecaseDiagram
    actor "Visitante (Anônimo)" as Visitante
    actor "Cliente (Autenticado)" as ClienteActor
    actor "Administrador" as Admin

    %% Navegação Pública
    Visitante --> (Visualizar Vitrine de Fontes)
    Visitante --> (Adicionar ao Carrinho de Compras)
    Visitante --> (Acessar Tela de Login)
    Visitante --> (Criar Cadastro de Cliente)

    %% Autenticação e Checkout
    ClienteActor --|> Visitante : "herda"
    ClienteActor --> (Realizar Finalização de Checkout)
    ClienteActor --> (Consultar Meus Pedidos Anteriores)

    %% Backoffice ADM
    Admin --> (Painel: Gerenciar Marcas)
    Admin --> (Painel: Gerenciar Modelos)
    Admin --> (Painel: Gerenciar Fontes do Catálogo)
    Admin --> (Painel: Gerenciar Fornecedores)
    Admin --> (Painel: Gerenciar Departamentos)
    Admin --> (Painel: Gerenciar Funcionários)
    
    %% Inclusões Obrigatórias
    (Painel: Gerenciar Fontes do Catálogo) ..> (Acessar Backend JWT Valido) : <<include>>
    (Realizar Finalização de Checkout) ..> (Acessar Backend JWT Valido) : <<include>>
```
